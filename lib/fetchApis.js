...
export async function getSliders() {
  const url = `${BASE_URL}/api/v1/posts?term_type=slider&order_by=order_column:asc`
...
export async function getProducts() {
  const url = `${BASE_URL}/api/v1/posts?term_type=products&order_by=order_column&order_direction=desc`
  const { ok, data } = await safeFetch(url, {
    next: { revalidate: 30 },
  })
...
export async function getCategories(type) {
  const url = `${BASE_URL}/api/v1/categories?taxonomy_type=${type}`

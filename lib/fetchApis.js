...
export async function getFeaturedProducts() {
  const url = `${BASE_URL}/posts?term_type=products&order_by=order_column:desc&per_page=18&is_featured=Yes`
  const { ok, data } = await safeFetch(url, {
    next: { revalidate: 30 },
  })
  return ok ? data?.data || [] : []
...
export async function getCategories(type) {
  const url = `${BASE_URL}/categories?taxonomy_type=${type}&limit=200&order_by=order_column&order_direction=asc`
...
export async function getBrands() {
  const url = `${BASE_URL}/brands?page=1&per_page=20`
  const { ok, data } = await safeFetch(url, {
    next: { revalidate: 30 },
  })
